...
Video usage is divided into four different types based on aggregate resolution and priced individually. After deducting the monthly <Link to={`${data.PATH}/reference/billing-policies#user-content-agoras-free-of-charge-policy-for-the-first-10000-minutes`}>10,000 free-of-charge minutes</Link> that {data.COMPANY} grants to every account, {data.COMPANY} multiplies any remaining usage by its corresponding unit price and adds up the costs to get the total cost for that month.
